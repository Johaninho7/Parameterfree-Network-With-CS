...
class parameterfreeNetwork:
	...
	def _determine_REF(self, transition_type):
		...
		elif transition_type == 'charge':
			# REF when transitioning from a CS node to an intermediate node
			return self._frt_cs
	...
	def _fdrive_delta(self, label, delta_l):
		...
		label.reqChargingTime += delta_l
		print(f"\nDRIVING for {delta_l} after leaving: {label.detailedPath[-1]}.")

	def _fbreak_delta(self, label):
		label.time += minTimeBreak
		label.elapsed_R += minTimeBreak
		label.drive_B = 0
		label.lBreak = minTimeBreak


	def _frest_delta(self, label):
		label.time += minTimeRest
		label.drive_R = 0
		label.elapsed_R = 0
		label.drive_B = 0
		label.lBreak = minTimeBreak
		label.rest = minTimeRest
		label.latest_R = float('inf')


	def _fvisit_nm(self, label, t_min_m, t_max_m, serviceTime_m, nextNodeId, dist_nm):	# Need two labels as input, currentLabel and newLabel, to accommodate l'_latest
	...
	def _frt_cs(self, label, delta_l):
		label.time = label.time if label.elapsed_R == 0 else (label.time - minTimeBreak if label.elapsed_R > 0 else label.time + label.reqCT)
		label.drive_B = 0
		label.elapsed_R = delta_l * alpha - minTimeBreak if label.elapsed_R == 0 else label.reqCT
		label.reqCT = 0
...
def _fbreak_delta(self, label, delta_l): # Replace delta with minTimeBreak
